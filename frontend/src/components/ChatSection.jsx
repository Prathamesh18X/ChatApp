...
const ChatSection = () => {
  ...
  const { groupMessages: groupMessages, loading: groupLoading } =
    useGetGroupMessages();
...
//   console.log("senderList : ",senderList);
  ...
  const senderColorMapping = {};
  
  for (let i = 0; i < senderList.length; i++) {
    const senderId = senderList[i];
    if (!(senderId in senderColorMapping)) {
      const randomIndex = Math.floor(Math.random() * colors.length);
      const randomColor = colors[randomIndex];
      senderColorMapping[senderId] = randomColor;
    }
  }
